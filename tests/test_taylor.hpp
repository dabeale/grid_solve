...
#ifndef TESTS_TEST_TAYLOR_HPP_
#define TESTS_TEST_TAYLOR_HPP_
...
int test_taylor_coefficients_squared() {
    ...
    for ( size_t i = 0; i < 10; ++i ) {
        for ( double testSigma : {1.0, 2.0, 5.0} ) {
            ...
            gs::vector<double, 3ul> yEvalPoint{
                10, 0, 0
...
int test_taylor_coefficients_inner() {
    ...
    //                         \          sigma^2           /
    // The left hand term on the right hand side is approximated,
    // and the other two terms are computed separately.
    //
    int retVal = 0;
    // Seed some randomness
    std::mt19937 gen(0);
