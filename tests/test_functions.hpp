    ...
    }
    {
        ...
        for ( size_t i = 0; i < 3; ++i ) {
            for ( size_t j = 0; j < 3; ++j ) {
                if ( i == j ) {
                    retVal += ASSERT_BOOL( std::abs(eVal(i, j) + 0.12886667007556424) < 1e-5);
                } else {
                    retVal += ASSERT_BOOL(std::abs(eVal(i, j) - 0.042955579924435765) < 1e-10);
                }
            }
        }
    }
    {
        ...
        for ( size_t i = 0; i < 3; ++i ) {
            for ( size_t j = 0; j < 3; ++j ) {
                for ( size_t k = 0; k < 3; ++k ) {
                    if (i == j && i == k) {
                        retVal += ASSERT_BOOL(std::abs(eVal(i, j, k) + 0.118128) < 1e-5);
                    } else if (i == j || i == k || k == j) {
                        retVal += ASSERT_BOOL(std::abs(eVal(i, j, k) + 0.0322167) < 1e-5);
                    } else {
                        retVal += ASSERT_BOOL(std::abs(eVal(i, j, k) - 0.0107389) < 1e-5);
                    }
                }
            }
        }
