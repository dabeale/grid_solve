...
int test_point_convert_tolocal_ind2sub() {
    std::cout << "Test boxes to local conversion ind2sub" << std::endl;
    const size_t level = 3;
    const size_t nDims = 2;
    int retVal = 0;
    gs::dimensions<nDims> dims(2, level);
    {
        auto type = gs::dimensions<nDims>::BOXES_SUBDIVISION;
        auto mode = gs::dimensions<nDims>::BOXES_MODE;
        auto conv = gs::dimensions<nDims>::LOCAL_CONV;
        {
            retVal += ASSERT_BOOL(dims.ind2sub(0, level, type, mode, conv ) == gs::index<nDims>({0, 0}, level));
            retVal += ASSERT_BOOL(dims.ind2sub(1, level, type, mode, conv ) == gs::index<nDims>({0, 1}, level));
            retVal += ASSERT_BOOL(dims.ind2sub(8, level, type, mode, conv ) == gs::index<nDims>({1, 0}, level));
        ...
        {
            retVal += ASSERT_BOOL(dims.ind2sub(4, level, type, mode, conv ) == gs::index<nDims>({0, 0}, level));
            retVal += ASSERT_BOOL(dims.ind2sub(5, level, type, mode, conv ) == gs::index<nDims>({0, 1}, level));
            retVal += ASSERT_BOOL(dims.ind2sub(12, level, type, mode, conv ) == gs::index<nDims>({1, 0}, level));
            retVal += ASSERT_BOOL(dims.ind2sub(13, level, type, mode, conv ) == gs::index<nDims>({1, 1}, level));
        }
    }
    {
        auto type = gs::dimensions<nDims>::POINTS_SUBDIVISION;
...
int test_point_convert_tolocal_sub2ind() {
    std::cout << "Test boxes to local conversion sub2ind" << std::endl;
    const size_t level = 3;
    const size_t nDims = 2;
    int retVal = 0;
    gs::dimensions<nDims> dims(2, level);
    {
        ...
        auto conv = gs::dimensions<nDims>::LOCAL_CONV;
        {
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({0, 0}, level), level, type, mode, conv ) == 0);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({0, 1}, level), level, type, mode, conv ) == 1);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({1, 0}, level), level, type, mode, conv ) == 2);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({1, 1}, level), level, type, mode, conv ) == 3);
        }
        {
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({2, 0}, level), level, type, mode, conv ) == 0);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({2, 1}, level), level, type, mode, conv ) == 1);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({3, 0}, level), level, type, mode, conv ) == 2);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({3, 1}, level), level, type, mode, conv ) == 3);
        }
        {
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({2, 2}, level), level, type, mode, conv ) == 0);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({2, 3}, level), level, type, mode, conv ) == 1);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({3, 2}, level), level, type, mode, conv ) == 2);
            retVal += ASSERT_BOOL(dims.sub2ind(gs::index<nDims>({3, 3}, level), level, type, mode, conv ) == 3);
        }
    }
    {
        auto type = gs::dimensions<nDims>::POINTS_SUBDIVISION;
