...
class analytic_multiply  {
    static constexpr size_t nBoxCorners = pow<2,M>();

    using grid_val = gs::vector_field<M,T,D>::grid_val;
    using box_val = gs::vector_field<M,T,D>::box_val;
    using box_corners = std::array<vector<T,M>, nBoxCorners>;
    using box_values = std::array<T, nBoxCorners>;
